      ...
      <div className="pt-28 pb-20 relative z-10">
        <div className="max-w-[1600px] mx-auto px-6 py-10">
          ...
          <div className="mb-6 text-center">
            <div className="inline-block p-4 bg-black/60 border border-emerald-400/30 rounded-xl">
              <h2 className="text-3xl font-bold text-emerald-400 font-mono tracking-wider">
                &gt;&gt; SKILL TREE &lt;&lt;
              </h2>
              <p className="text-white/70 mt-2">Completed development skills and mastered abilities</p>
            </div>
            
            {/* Skill Tree Stats */}
            <div className="flex justify-center gap-6 mt-6">
              <div className="bg-black/60 border border-emerald-400/30 rounded-lg p-4">
                <div className="text-2xl font-bold text-emerald-400 font-mono">{totalAchievements}</div>
                <div className="text-xs text-white/60 uppercase tracking-widest">NODES UNLOCKED</div>
              </div>
              <div className="bg-black/60 border border-amber-400/30 rounded-lg p-4">
                <div className="text-2xl font-bold text-amber-400 font-mono">{totalXP.toLocaleString('en-US')}</div>
                <div className="text-xs text-white/60 uppercase tracking-widest">SKILL POINTS</div>
              </div>
            </div>
          </div>
              ...
              <div
                ...
                className={`relative rounded-xl border-2 p-4 transition-all duration-300 cursor-pointer z-10 ${
                  selectedNode === categoryName 
                    ? 'border-emerald-400/60 bg-emerald-500/20 shadow-2xl transform scale-105' 
          ...
          {selectedNode && skillTree[selectedNode as keyof typeof skillTree] && (
            <div className="mt-8 bg-black/80 border border-cyan-400/30 rounded-xl p-6">
              <h3 className="text-2xl font-bold text-cyan-400 font-mono mb-4">{selectedNode}</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {skillTree[selectedNode as keyof typeof skillTree].achievements.map((achievement) => (
                  <div key={achievement.id} className="bg-gray-900/60 border border-gray-600 rounded-lg p-4">
                    <div className="mb-3">
                      <div className="text-cyan-400 font-bold text-sm">{achievement.title}</div>
                      <div className="text-purple-400 font-bold text-xs">+{achievement.xp} SP</div>
                    </div>
                    <div className="text-gray-300 text-sm">{achievement.desc}</div>
                    <div className="mt-3 text-emerald-400 font-bold text-xs text-center">✓ COMPLETE</div>
                  </div>
                ))}
              </div>
            </div>
