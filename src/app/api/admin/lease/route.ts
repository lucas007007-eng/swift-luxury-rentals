...
export async function POST(req: Request) {
  try {
    ...
    yTmp = measurePara(`Phone No.: ${(b as any).clientPhone || '—'}`, yTmp, colWidth)
    yTmp = measurePara(`E-Mail: ${(b as any).clientEmail || '—'}`, yTmp, colWidth)
    drawSectionBox(leftX, yL, yTmp)
    const s2L = yL; yL = wrapHeading('2. Mieter / Tenant', leftX+2, yL, colWidth-4)
    yL = wrapPara(`${b.clientName}`, leftX, yL, colWidth)
    yL = wrapPara(`Phone No.: ${(b as any).clientPhone || '—'}`, leftX, yL, colWidth)
    yL = wrapPara(`E-Mail: ${(b as any).clientEmail || '—'}`, leftX, yL, colWidth)
    yTmp = yR
    ...
    yR = wrapPara(`Phone No.: ${(b as any).clientPhone || '—'}`, rightX, yR, colWidth)
    yR = wrapPara(`E-Mail: ${(b as any).clientEmail || '—'}`, rightX, yR, colWidth)
    ensureSpace()
