          ...
          <div className="rounded-xl border border-emerald-400/30 bg-gradient-to-br from-[#0b1a12] to-[#08120d] p-4">
            ...
            {adding && (
              <div className="mt-4 grid grid-cols-1 md:grid-cols-5 gap-3">
                <input placeholder="Client name" className="bg-[linear-gradient(145deg,#0a0a0a_0%,#1a1a1a_50%,#0a0a0a_100%)] border border-zinc-400/30 rounded px-2 py-1 text-sm text-white placeholder-zinc-400 shadow-[0_4px_12px_rgba(0,0,0,0.4),inset_0_1px_0_rgba(255,255,255,0.1)]" value={form.clientName} onChange={(e)=>setForm({...form, clientName: e.target.value})} />
                <select className="bg-[linear-gradient(145deg,#0a0a0a_0%,#1a1a1a_50%,#0a0a0a_100%)] border border-zinc-400/30 rounded px-2 py-1 text-sm text-white shadow-[0_4px_12px_rgba(0,0,0,0.4),inset_0_1px_0_rgba(255,255,255,0.1)]" value={form.propertyId} onChange={(e)=>setForm({...form, propertyId: e.target.value})}>
                  ...
                  {Object.entries(cityProperties).flatMap(([c, list])=> list.map((p:any)=> (
                    <option key={p.id} value={p.id} className="bg-black text-white">{c} — {p.title}</option>
                  )))}
                </select>
