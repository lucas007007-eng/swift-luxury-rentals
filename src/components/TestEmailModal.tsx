        ...
        {testResult && (
          ...
          >
            ...
            <div className="bg-white rounded-lg p-4 mb-3 space-y-3">
              <h4 className="font-medium text-gray-900">📊 Delivery Analytics</h4>
              
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <span className="text-gray-500">Email ID:</span>
                  <span className="ml-2 font-mono text-gray-900 text-xs">
                    {testResult.emailId?.substring(0, 12)}...
                  </span>
                </div>
                <div>
                  <span className="text-gray-500">Status:</span>
                  <span className={`ml-2 font-semibold ${
                    testResult.deliveryStatus === 'sent' ? 'text-green-600' : 'text-red-600'
                  }`}>
                    {testResult.deliveryStatus.toUpperCase()}
                  </span>
                </div>
                <div>
                  <span className="text-gray-500">Provider:</span>
                  <span className="ml-2 text-gray-900">
                    {testResult.analytics?.provider || 'Resend'}
                  </span>
                </div>
                <div>
              ...
              {testResult.resendResponse && (
                <div className="mt-3 p-3 bg-gray-50 rounded-lg">
                  <p className="text-xs text-gray-600 mb-1">Resend Response:</p>
                  <div className="flex items-center gap-2">
                    <span className={`w-2 h-2 rounded-full ${
                      testResult.resendResponse.status === 'sent' ? 'bg-green-500' : 'bg-red-500'
                    }`}></span>
                    <span className="text-sm font-mono">
                      {testResult.resendResponse.id || 'No ID returned'}
                    </span>
                  </div>
                </div>
              )}
